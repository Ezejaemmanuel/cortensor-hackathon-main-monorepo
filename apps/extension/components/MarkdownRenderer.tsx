...
export const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({ 
  ...
  return (
    <div className={cn('max-w-none prose prose-slate dark:prose-invert prose-lg', className)}>
      <ReactMarkdown
        remarkPlugins={[remarkGfm]}
        components={{
          // Enhanced Headings with better typography
          h1: ({ children }) => (
            <h1 className="pb-3 mt-8 mb-6 text-3xl font-bold leading-tight border-b text-slate-900 dark:text-slate-100 border-slate-200 dark:border-slate-700">
              {children}
            </h1>
          ),
          h2: ({ children }) => (
            <h2 className="mt-6 mb-4 text-2xl font-semibold leading-tight text-slate-800 dark:text-slate-200">
              {children}
            </h2>
          ),
          h3: ({ children }) => (
            <h3 className="mt-5 mb-3 text-xl font-semibold leading-tight text-slate-800 dark:text-slate-200">
              {children}
            </h3>
          ),
          h4: ({ children }) => (
            <h4 className="mt-4 mb-2 text-lg font-medium text-slate-700 dark:text-slate-300">
              {children}
            </h4>
          ),
          ...
          ul: ({ children }) => (
            <ul className="pl-6 mb-4 space-y-2 text-slate-700 dark:text-slate-300">
              {children}
            </ul>
          ),
          ol: ({ children }) => (
            <ol className="pl-6 mb-4 space-y-2 text-slate-700 dark:text-slate-300">
              {children}
            </ol>
          ),
          li: ({ children }) => (
            <li className="relative pl-2">
              <span className="absolute top-0 -left-6 text-slate-400 dark:text-slate-500">•</span>
              {children}
            </li>
          ),
          ...
          blockquote: ({ children }) => (
            <blockquote className="py-4 pr-4 pl-6 my-6 rounded-r-lg border-l-4 border-blue-500 bg-slate-50 dark:bg-slate-800/50">
          ...
          // Enhanced Tables
          table: ({ children }) => (
            <div className="overflow-x-auto my-6 rounded-lg border shadow-sm border-slate-200 dark:border-slate-700">
          ...
          th: ({ children }) => (
            <th className="px-6 py-3 text-xs font-semibold tracking-wider text-left uppercase text-slate-600 dark:text-slate-400">
              {children}
            </th>
          ),
          td: ({ children }) => (
            <td className="px-6 py-4 text-sm whitespace-nowrap border-t text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700">
              {children}
            </td>
          ),
          ...
          hr: () => (
            <hr className="my-8 h-px bg-gradient-to-r from-transparent to-transparent border-0 via-slate-300 dark:via-slate-600" />
          ),
